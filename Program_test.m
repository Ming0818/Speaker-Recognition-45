...
folder1 = dir('D:\Acads\IDP-sem7\codes\model_12_MFCC'); %Directory having the speaker models
%Loading the speaker models
for i =1:N
    fname=folder1(i+2).name;
    fpath=strcat('D:\Acads\IDP-sem7\codes\model_12_MFCC\',fname);
    load(fpath);
    S(i) = model1; %Holds all the speaker models
end
%Testing files
...
T = [0.1 0.5 0.8 1 2 5];%Test sample length in seconds
...
for i = 1:N
    ...
    for j = 1:numel(T)
        ...
        for k = 1: N
            loglikelihood(k,:,j) = loglikelihood_cal(coeff(:,1:M+n-1),S(k),N,M,n);%Function to find the log likelihood
        end
